...
from observability import logger_provider, logger, provider
...
# metrics
meter = metrics.get_meter("fraud.analysis", "1.0.0")
# gauge_observer = meter.create_observable_gauge("fraud.up_down_counter_observer", description="Fraud Analysis")


# Python
# UpDownCounter
counter = meter.create_counter("Fraud_Counter", description="Fraud Alert")

# def gauge_observer():
#     def observable_up_down_counter_func(result,event_key) -> [Observation]:
#             return [Observation(result, {"ID": event_key})]
#
#         counter = meter.create_observable_up_down_counter(
#                 "fraud.up_down_counter_observer",
...
def preprocess_data(data, counter):
    ...
    df.drop(['first', 'last', 'job', 'dob', 'street', 'city', 'zip', 'trans_num', 'state', 'unix_time', 'gender'],
            axis=1, inplace=True)
    # extracting useful columns
    trans_datetime = df['trans_date_trans_time'][0]
    trans_amount = df['amt'][0]
    ...
    c_result = rfm_calculate.get_customer_spending_behaviour_features(
        existing_df[existing_df["cc_num"].values == e_df["cc_num"].values])
    ...
    n_df['distance'] = n_df.apply(lambda row: rfm_calculate.calculate_distance(row["cust_locn"], row["merchant_locn"]),
                                  axis=1)
    # Dropping columns which are not required for model
    n_df.drop(['trans_date_trans_time', 'cc_num', 'merchant', "category", "merchant_locn", "cust_locn",
               "Frequency_7DAY_WINDOW",
               "Frequency_30DAY_WINDOW", "Monetary_30DAY_WINDOW", "merchant_Trnscn_count7Day",
               "merchant_Trnscn_count30Day",
               "merchant_Risk_Score30Day", "category_Trnscn_count7Day", "category_Risk_Score7Day",
               "category_Trnscn_count30Day",
               "category_Risk_Score30Day"], axis=1, inplace=True)
    final_df = converting_dtype(n_df)
    # print(final_df.info())
    result = int(model.combined_predctns(final_df))

    if result == 1:  # fraud exists
        counter.add(1, {"ID": event_key})
        send_email(event_key, str(trans_amount), str(trans_datetime))

    update_db(result, event_key)


def sample_consumer():
    consumer = KafkaConsumer('testf', bootstrap_servers=['localhost:9092'], auto_offset_reset='earliest',
    ...
    for message in consumer:
        ...
        print(message)
        preprocess_data(message, counter)
